...
async def render_batches(args: argparse.Namespace) -> None:
    ...
    log_event(
        "starting render | plans=%d | clip_batches=%d | output_root=%s | manifest=%s | server=%s | separation_cache=%s | max_audio_seconds=%s | shuffle=%s | seed=%d"
        % (
            total_plans,
            len(plan_batches),
            output_root,
        ...
        async with ClientSession(read, write) as session:
            ...
            with temp_manifest_path.open("w", encoding="utf-8") as manifest_handle:
                for batch_index, batch in enumerate(plan_batches, start=1):
                    first_row = batch[0]
                    log_event(
                        "starting clip batch %d/%d | clip_id=%s | plans=%d | audio_path=%s"
                        % (
                            batch_index,
                            len(plan_batches),
                            first_row.get("clip_id", "unknown_clip"),
                            len(batch),
                            first_row.get("audio_path"),
                        )
                    )
                    for row in batch:
                        plan_id = str(row.get("plan_id"))
                        ...
                        audio_path = row.get("audio_path")
                        try:
                            if args.skip_harmony and graph_has_operator(list(row["graph_spec"]), "apply_harmony_effect"):
                            ...
                            failures += 1
                            if printed_errors < 20:
                                print(
                                    "ERROR plan_id=%s clip_id=%s audio_path=%s :: %s"
                                    % (plan_id, clip_id, audio_path, error),
                                    file=sys.stderr,
                                    flush=True,
                                )
                                printed_errors += 1
